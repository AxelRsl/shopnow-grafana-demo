...
param(
    ...
    [ValidateSet('smoke', 'baseline', 'black-friday', 'frontend')]
    [string]$Test = 'smoke',
...
try {
    ...
    Write-Host "[OK] K6 installed: $k6Version" -ForegroundColor Green
} catch {
...
$scriptPath = switch ($Test) {
    ...
    'frontend'     { ".\scripts\frontend-test.js" }
}
...
if ($Cloud) {
    Write-Host "[MODE] K6 Cloud" -ForegroundColor Yellow
} else {
...
try {
    ...
    Write-Host "[ERROR] API Gateway is not responding!" -ForegroundColor Red
...
if ($Cloud) {
    # Run with K6 Cloud
    Write-Host "Uploading to K6 Cloud..." -ForegroundColor Cyan
    k6 cloud $scriptPath
} else {
    # Run locally
    k6 run $scriptPath
...
Write-Host ""
Write-Host "View results in Grafana Cloud:" -ForegroundColor Cyan
